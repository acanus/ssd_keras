...
from models.keras_mobilenet_ssd300 import ssd_mobilenet_300
...
# model_path = 'local/trained/model.h5'
...
train_dataset.create_hdf5_dataset(file_path='local/dataset_udacity_traffic_train.h5',
                                  resize=False,
                                  variable_image_size=True,
                                  verbose=True)

val_dataset.create_hdf5_dataset(file_path='local/dataset_udacity_traffic_val.h5',
                                resize=False,
                                variable_image_size=True,
                                verbose=True)
...
model_checkpoint = ModelCheckpoint(filepath='local/ssd300_pascal_07+12_epoch-{epoch:02d}_loss-{loss:.4f}_val_loss-{val_loss:.4f}.h5',
                                   monitor='val_loss',
                                   verbose=1,
                                   save_best_only=True,
...
csv_logger = CSVLogger(filename='local/ssd300_pascal_07+12_training_log.csv',
                       separator=',',
                       append=True)
...
train=True
if(train):
    model = ssd_mobilenet_300(image_size=(img_height, img_width, img_channels),
                n_classes=n_classes,
                mode='training',
